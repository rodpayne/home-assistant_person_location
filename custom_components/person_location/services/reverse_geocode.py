...
async def _async_call_google_maps_geocoding(
    ...
    if "results" in google_decoded and google_decoded["results"]:
        ...
        if formatted_address:
            ...
            target._attr_extra_state_attributes[ATTR_GOOGLE_MAPS] = formatted_address
    ...
    target._attr_extra_state_attributes[ATTR_ATTRIBUTION] += google_attribution + "; "
...
async def _async_call_mapquest_reverse_geocoding(
    pli: PersonLocationIntegration,
    target: PersonLocationTargetSensor,
    ctx: ReverseGeocodeContext,
) -> None:
    ...
    _LOGGER.debug("(%s) mapquest formatted_address = %s", entity_id, formatted_address)
    ...
    target._attr_extra_state_attributes[ATTR_ATTRIBUTION] += mapquest_attribution + "; "
...
async def _async_call_open_street_map_reverse_geocoding(
    pli: PersonLocationIntegration,
    target: PersonLocationTargetSensor,
    ctx: ReverseGeocodeContext,
) -> None:
    ...
    target._attr_extra_state_attributes[ATTR_OPEN_STREET_MAP] = display_name.replace(
        ", ", " "
    )
    ...
    if "licence" in osm_decoded:
        ...
        target._attr_extra_state_attributes[ATTR_ATTRIBUTION] += osm_attribution + "; "
...
async def _async_call_radar_reverse_geocoding(
    pli: PersonLocationIntegration,
    target: PersonLocationTargetSensor,
    ctx: ReverseGeocodeContext,
) -> None:
    ...
    target._attr_extra_state_attributes[ATTR_ATTRIBUTION] += radar_attribution + "; "
...
async def _handle_reverse_geocode(pli: PersonLocationIntegration, call: dict) -> bool:
    """
    ...
    try:
        ...
        async with pli.target_lock(entity_id):
            ...
            old_latitude = target.this_entity_info.get(INFO_LOCATION_LATITUDE, "None")
            old_longitude = target.this_entity_info.get(INFO_LOCATION_LONGITUDE, "None")
            ...
            if ctx.new_latitude == "None" or ctx.new_longitude == "None":
                ...
                _LOGGER.debug("(%s) elapsed_seconds = %s", entity_id, elapsed_seconds)
                ...
                if is_provider_enabled(pli.hass, SWITCH_RADAR_GEOCODING_API):
                    await _async_call_radar_reverse_geocoding(pli, target, ctx)
                else:
                    previous = target._attr_extra_state_attributes.pop(ATTR_RADAR, None)
                    if previous:
                        _LOGGER.debug(
                ...
                # ------- OpenStreetMap -----------------------------------
                if is_provider_enabled(pli.hass, SWITCH_OSM_NOMINATIM_GEOCODING_API):
                    await _async_call_open_street_map_reverse_geocoding(
                        pli,
                        target,
                        ctx,
                    )
                ...
                if is_provider_enabled(pli.hass, SWITCH_GOOGLE_GEOCODING_API):
                    await _async_call_google_maps_geocoding(pli, target, ctx)
                else:
                    previous = target._attr_extra_state_attributes.pop(
                ...
                if is_provider_enabled(pli.hass, SWITCH_MAPQUEST_GEOCODING_API):
                    await _async_call_mapquest_reverse_geocoding(pli, target, ctx)
                else:
                    previous = target._attr_extra_state_attributes.pop(
                        ATTR_MAPQUEST, None
                    )
                ...
                # ------- All ---------------------------------------------
                target._attr_extra_state_attributes[ATTR_LOCALITY] = ctx.new_locality
            ...
            if ATTR_ZONE in target._attr_extra_state_attributes:
                ...
                current_zone_obj = pli.hass.states.get(f"{ZONE_DOMAIN}.{current_zone}")
            ...
            if ATTR_BREAD_CRUMBS in target._attr_extra_state_attributes:
                ...
                if not old_bread_crumbs.endswith(new_bread_crumb):
                    target._attr_extra_state_attributes[ATTR_BREAD_CRUMBS] = (
                        shorten_to_last_255(old_bread_crumbs + "> " + new_bread_crumb)
                    )
            else:
            ...
            friendly_name_variables = {
                ...
                "person_name": target._attr_extra_state_attributes.get("person_name"),
                "source": {
...
async def async_setup_reverse_geocode(pli: PersonLocationIntegration) -> bool:
    ...
    pli.hass.services.async_register(
        DOMAIN,
        "reverse_geocode",
        partial(_handle_reverse_geocode, pli),
    )
    return True
